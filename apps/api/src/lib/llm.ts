...
/** Freeform chat completion (focus coach, conversational replies). */
export async function chatWithLLM(input: {
  ...
  const apiKey = resolveApiKey();
  if (!apiKey) {
    console.error("chatWithLLM skipped: missing Bedrock/LiteLLM API key");
    return null;
  }

  try {
    const res = await getLlmClient().chat.completions.create({
      model: resolveLlmModel(input.kind || "default"),
      messages: [
    ...
    console.error("chatWithLLM failed:", err);
    return null;
  }
}
