...
export async function parseDateFromUserText(
  ...
  // Today in the user's zone — helps the model resolve "tomorrow", "next Friday", etc.
  const nowInZone = DateTime.now().setZone(zone);
  const todayIso = nowInZone.toISODate() || nowInZone.toFormat("yyyy-MM-dd");

  const extracted = await extractWithLLM(`
You are DeadlineAI's date understanding model. The user is chatting naturally in Telegram.
Extract the intended deadline from their message — they do NOT need a specific format.

User timezone (IANA): ${zone}
Today in that timezone: ${todayIso} (${nowInZone.toFormat("CCCC, LLL d, yyyy")})
User message: ${JSON.stringify(input)}

Understand casual / ChatGPT-style phrasing, including:
- "set 25th july 2026", "make it July 25", "deadline is 25 July"
- "2026-07-25 11:59 PM IST"
- "tomorrow 5pm", "next Monday", "in 3 days", "end of this month"
- "by Friday evening", "before 20 May", "last date 25th May night"
- typos, ordinals (25th), and mixed languages if the date is still clear

Rules:
- If ONLY a calendar date is given (no clock time), use END of that day (23:59:59.999) in ${zone}.
- If a time is given, use that time in ${zone}, unless they named another zone (IST, PST, etc.).
- Relative phrases ("tomorrow", "next week") are relative to Today above.
- Prefer the most likely single deadline; do not invent a date when none is present.
